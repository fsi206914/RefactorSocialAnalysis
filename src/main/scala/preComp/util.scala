...
object Util {
  ...
  def printResult(fileName: String, resultMap: scala.collection.mutable.HashMap[(Int, Int), (Double, Double, Double, Int, Int, Int)],
    baselineResult: Double, baselineResult2: Double, satResult: Double) =
      printToFile(new java.io.File(fileName))(p => {
        resultMap.foreach(res => p.println( "src1 and src2 = " + res._1._1 + "\t"+ res._1._2 + "\t" + " probSAT =" +  res._2._1
        +"\t" + " prob_CFBaseline =" +  res._2._2 + " prob_CCBaseline =" +  res._2._3 + "\t"  + " mutual Frd num=" + res._2._4 +  "\t" + " mutual Comm num=" + res._2._5 + " fiveSet size=" + res._2._6))
        p.println("additionCFBaseline sqrt = " + baselineResult )
        p.println("additionCCBaseline sqrt = " + baselineResult2 )
        p.println("additionProbSAT sqrt = " + satResult )
      })
