...
object MCSAT {
  ...
  def apply(frdsMap: Map[Int, ArrayBuffer[Int]], commMap: Map[Int, ArrayBuffer[Int]])(src1: Int, src2: Int, satNum: Int) = {
    ...
    println("If the the two people know each other = " + counter.toFloat/satNum)
    val numMutualFrds = findNumMutualFrdsLocal(src1, src2);
    val numComm = findNumMutualComms(src1, src2);

    // if(numMutualFrds == 0)
    //   (counter.toFloat/conf.getInt("MCSATSampleNum"), 0.0);
    // else 
      (counter.toFloat/satNum, probCommonFrd(numMutualFrds), probCommonComm(numComm));
  }
